    ...
    IMapper mapper) : IAppointmentService
{
    public async Task<Result<string>> CompleteAsync(CompleteAppointmentDto request, CancellationToken cancellationToken)
    {
        Appointment? appointment = 
            ...
            .GetByExpressionWithTrackingAsync(p => p.Id == request.AppointmentId, cancellationToken); //GetByExpressionda tracking mekanizması açık geliyor ve tekrar update yazmamıza gerek olmuyor.
        ...
        {
            return Result<string>.Failure("Appointment is already closed. You cannot close again");
        }
    ...
    public async Task<Result<string>> CreateAsync(CreateAppointmentDto request, CancellationToken cancellationToken)
    {
        ...
        IQueryable<Appointment> appointments = appointmentRepository
        ...
        return Result<string>.Succeed("Create appointment is succeeded");
    ...
    public async Task<Result<string>> DeleteByIdAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        return Result<string>.Succeed("Appointment deletion is successful");
    }
    ...
    public async Task<Result<List<User>>> GetAllDoctorsAsync(CancellationToken cancellationToken)
    {
        var doctors =
            await userManager
            .Users
            .Where(p => p.UserType == UserType.Doctor).Include(p => p.DoctorDetail).OrderBy(p => p.FirstName).ToListAsync(cancellationToken);
