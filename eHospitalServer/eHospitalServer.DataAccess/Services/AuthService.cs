    ...
    JwtProvider jwtProvider) : IAuthService
{
    ...
    {
        User? user = await userManager.Users.Where(p => p.EmailConfirmCode == emailConfirmCode).FirstOrDefaultAsync
            (cancellationToken);
        if (user is null)
        {
            return Result<string>.Failure(500, "Email confirm code is not valid");
        }
        ...
        return Result<string>.Succeed("Email verification successfull");
    }
    ...
    public async Task<Result<string>> SendConfirmEmailAsync(string email, CancellationToken cancellationToken)
    {
        ...
        {
            return Result<string>.Failure(500, "Verification email is sent every 3 minutes.");
        }
    ...
    public async Task<Result<string>> ChangePasswordWithForgotPasswordCodeAsync(ChangePasswordWithForgotPasswordCodeDto request, CancellationToken cancellationToken)
    {
       ...
       IdentityResult result = await userManager.ResetPasswordAsync(user, token, request.NewPassword);
        ...
        return Result<string>.Succeed("Your Password is changed successfully. You can Sign in using your new password.");
    }
    public async Task<Result<string>> SendForgotPasswordEmailAsync(string emailOrUserName, CancellationToken cancellationToken)
    {
        ...
        while (isForgotPasswordCodeExists)
        {
            ...
            isForgotPasswordCodeExists = await userManager.Users.AnyAsync(p => p.ForgotPasswordCode == forgotPasswordCode,
                cancellationToken);
        }
        ...
        #region Send Password Recovery - Reset Code
    ...
    private string MaskEmail(string email)
    {
        ...
        if (atIndex == -1) return email; // Geçerli bir e-post adresi değilse, değişiklik yapmadan döndür
        ...
        return maskedUsername + "@" + domain; // E-post adresinde nokta yoksa veya tanımlamazsa.
    }
